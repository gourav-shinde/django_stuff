...
def quiz_jview(request):
	...
	qe=QuizSerializer(quiz_expired,many=True)
	qa=QuizSerializer(quiz_active,many=True)
	data={'quiz_active':qa.data,'quiz_expired':qe.data}
...
def question_view(request,my_id,num=0,flag=0):
	...
	if request.POST:
		...
		optno=request.POST.getlist('correct')
...
@api_view(['GET'])
def quiz_student_list(request):
	quizzes=QuizTaker.objects.filter(user=request.user).order_by('quiz__roll_out_time')
	now = timezone.now()
	#print(now)
	quiz=quizzes.filter(completed=False,quiz__stop_time__gte=now)
	quiz_completed=quizzes.filter(Q(completed=True)|Q(quiz__stop_time__lte=now))
	quiz_remain,quiz_complete=[],[]
	for x in quiz:
		quiz_remain.append({'id':x.id,'lecture':x.quiz.lecture.subject,'name':x.quiz.name,'time':x.quiz.roll_out_time,'stop':x.quiz.roll_out_time,'slug':x.quiz.slug})
	for x in quiz_completed:
		quiz_complete.append({'id':x.id,'lecture':x.quiz.lecture.subject,'name':x.quiz.name,'time':x.quiz.roll_out_time,'stop':x.quiz.roll_out_time,'slug':x.quiz.slug,'score':x.score})
	data={"ongoing":quiz_remain,"expired":quiz_complete}
